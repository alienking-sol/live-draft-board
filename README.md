...
![The draft board as it appears on the TV during the draft](docs/draft-board.png)
...
![Best available, position runs, and roster holes](docs/war-room.png)
...
Every player, team, and manager name in `mockups/` and `docs/` is invented. The mockups
are generated by [`scripts/build_mockups.py`](scripts/build_mockups.py) from a hardcoded
fictional draft, so this repository contains no Yahoo Fantasy Sports data. Regenerate
them with:
...
Open `mockups/draft-board.html` or `mockups/war-room.html` directly in a browser to see
the layouts at full size.

## Status

Design and specification complete. Implementation is blocked on API access approval.
